...
def verify_dataset_integrity():
    ...
    # 4. Memverifikasi konsistensi antara dataset klinis dan sinyal
    print("\n4. Verifying consistency between clinical data and signals...")
    
    # Mendapatkan daftar record dari dataset klinis
    clinical_records = set(mature_df['record_name'].astype(str))
    
    # Mendapatkan daftar record dari file sinyal
    signal_records = set([f.split('_')[0] for f in signal_files])
    
    # Memverifikasi apakah semua record klinis memiliki file sinyal
    missing_signals = clinical_records - signal_records
    extra_signals = signal_records - clinical_records
    
    print(f"   Clinical records: {len(clinical_records)}")
    print(f"   Signal records: {len(signal_records)}")
    print(f"   Missing signal files: {len(missing_signals)}")
    print(f"   Extra signal files: {len(extra_signals)}")
